...
type RuntimeBlock struct {
	...
	OS   string // runtime.GOOS：如 "darwin" / "windows" / "linux"
	Arch string // runtime.GOARCH：如 "arm64" / "amd64"

	// Machine identity. (both)
	Hostname string // os.Hostname()

	// CPU. (both)
	CPUCount         int    // runtime.NumCPU()：进程可用的 logical CPU 数
	CPUCoresPhysical int    // 物理核心数；mac: sysctl hw.physicalcpu
	CPUModel         string // 型号字串；mac: sysctl machdep.cpu.brand_string

	// Memory. (both)
	MemoryBytes int64 // 物理总内存字节数；mac: sysctl hw.memsize

	// OS version / build. (both)
	OSVersion     string // 用户可读版本号；mac: "14.x"；win: "11 x"
	OSBuild       string // 细粒度 build 号；mac: "23xxx"；win: "22xxx.xxx"
	KernelVersion string // 内核版本；mac: uname -r；win: NT 版本号

	// Linux-only hints. (linux; mac/win 留空)
	Distro      string // Linux 发行版标识
	SessionType string // XDG_SESSION_TYPE：wayland / x11 / tty

	// Virtualization / emulation hint. (all; TASK-105 / Phase 2 实现)
	Virtualization string // 可能值："rosetta" / "wsl" / "docker" / "vm" / ""
}
...
type ProcessBlock struct {
	// Kernel process IDs.
	PID  int // (both) Process ID：本进程在 kernel 里的唯一编号
	PPID int // (both) Parent PID：fork/exec（mac）/ CreateProcess（win）本进程的父进程
	PGID int // (mac)  Process Group ID：进程组号，用于信号投递与前后台作业控制；win 留 -1
	SID  int // (mac)  Unix Session ID：进程组的集合，绑定控制终端；win 留 -1（Windows 登录会话见 WindowsSessionID）

	// Unix numeric user / group identity. (mac; win 留 -1)
	UID  int // real user ID：启动本进程的用户
	EUID int // effective user ID：权限判定实际使用的 UID（setuid 后可与 UID 不同）
	GID  int // real group ID：用户主组
	EGID int // effective group ID：权限判定实际使用的 GID

	// 跨平台的可读 user / group 名. (both)
	Username  string // 登录名；mac: 例如 "alice"；win: "DOMAIN\\user" 或 "user"
	Groupname string // 主组名；mac: 例如 "staff"；win: 若可解析为可读名，否则为空

	// Windows-specific identity. (win; mac 留空)
	WindowsUserSID        string // 用户 SID，如 "S-1-5-21-..."
	WindowsGroupSID       string // 主组 SID
	WindowsSessionID      int    // 登录会话号；0=services session；1+ 为交互登录；mac 留 -1
	WindowsDomain         string // 登录域；即 "DOMAIN\\user" 中 DOMAIN 部分
	WindowsIntegrityLevel string // 进程完整性等级：Low / Medium / High / System
	WindowsElevated       bool   // 是否以管理员运行；对应 token 的 Elevation 标志

	// Invocation shape. (both)
	Argv []string // 完整命令行参数（含 argv[0]）；nil 表示不可用
	CWD  string   // current working directory；"<err: ...>" 表示 os.Getwd() 失败

	// Time. (both；两边都需平台代码实现，stdlib 不直接给)
	StartTime time.Time // 本进程启动时刻（wall-clock）
}

// ChainBlock 是 Schema v1 中 chain 分组的 Go 映射：
...
type ChainBlock struct {
	Depth       int          // len(Layers) 冗余存，便于快速筛选 / 诊断
	Termination string       // 见上方枚举：init / cycle / query_failed / orphan / depth_cap
	Layers      []ChainLayer // 从 index 0（直接父进程）开始向上
}

// ChainLayer 祖先链单层快照：仅保存 ps 直接返回的原始字段。
//
...
type ChainLayer struct {
	...
	PID  int // 本层 pid
	PPID int // 本层父 pid（决定下一跳）
	PGID int // (mac) 进程组号；win 留 -1
	SID  int // (mac) Unix Session ID；win 留 -1

	// Identity. (mac)
	UID      int    // 本层实际运行用户 uid；win 留 -1
	Username string // ps -o user= 输出

	// Invocation shape.
	Comm    string // 可执行 basename（ps -o comm=）；登录 shell 会带 "-" 前缀
	Command string // 完整 argv 字串（ps -o command=）
	TTY     string // 控制终端，如 "ttys003"；"??" 表示无 tty

	// Time (ps 原样字符串；time.Time 解析留给消费者).
	Lstart string // 如 "Sun Apr 19 12:00:00 2026"

	// Raw fallback: 防解析失败丢信息.
	RawPsLine string // ps 整行输出原样
}

// EnvBlock 环境变量快照：按 family 分组 + 兜底 Uncategorized + 全量 Raw。
//
// 设计原则：
...
type EnvBlock struct {
	// 12 个 family，按特异性从窄到宽排列（和匹配优先级一致）
	...
	ShellVersion  map[string]string // BASH_VERSION / ZSH_VERSION / FISH_VERSION / PSModulePath
	...
	User          map[string]string // Unix: USER / LOGNAME / HOME / TMPDIR；Windows: USERPROFILE / APPDATA / LOCALAPPDATA / USERDOMAIN / TEMP / TMP
